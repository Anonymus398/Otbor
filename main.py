...
def encrypt(pac, lo):  # Определение функции encrypt с параметрами pac и lo
    pyAesCrypt.encryptFile(str(lo) + ".db", str(lo) + ".db.aes", pac)  # Шифрование файла
...
async def main(page: ft.Page):  # Определение асинхронной функции main с параметром page
    ...
    page.horizontal_alignment = ft.CrossAxisAlignment.CENTER  # Установка горизонтального выравнивания
    page.scroll = ft.ScrollMode.ADAPTIVE  # Установка режима прокрутки
    await page.add_async(TodoApp())  # Асинхронное добавление объекта TodoApp на страницу
