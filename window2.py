...
# Расшифровка файла
def decrypt(lo, pa):
    pyAesCrypt.decryptFile(str(lo) + ".db.aes", str(lo) + ".db", pa.value)  # Расшифровка файла
...
def main(page: ft.Page):
        login_txt = ft.TextField(label="Логин", width= 400, autofocus=True)  # Создание текстового поля для ввода логина
        password = ft.TextField(label="Пароль", width= 400, password=True)  # Создание текстового поля для ввода пароля
        ...
        def reg_button(e):
            ...
            with sqlite3.connect('us.db') as conn:  # Установление соединения с базой данных
                ...
                if notes != []:  # Проверка наличия пользователей в базе данных
                        ...
                        curs.execute('INSERT INTO users (user, hesh, salt) VALUES (?, ?, ?)', (str(log), str(hashe), str(salt)))  # Добавление нового пользователя в базу данных
            ...
            with sqlite3.connect(str(log) + '.db') as conn:  # Установление соединения с базой данных
                curs = conn.cursor()  # Создание объекта курсора
                curs.execute('''
                ...
                pyAesCrypt.encryptFile(str(log) + ".db", str(log) + ".db.aes", p)  # Шифрование файла
            conn.close()  # Закрытие соединения с базой данных
        ...
        button2 = ft.TextButton("Регистрация", on_click=on_text_button_click)  # Создание кнопки "Регистрация"
        text = ft.Text("Такой пользователь уже существует")  # Создание текста об ошибке
        button3 = ft.ElevatedButton("Регистрация", on_click=reg_button)  # Создание кнопки "Регистрация"
        page.add(login_txt, 
                 password, 
                 button1,
                 button2, 
